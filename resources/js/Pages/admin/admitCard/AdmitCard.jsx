...
// import ResultGetData from "./ResultGetData";
...
//         </table>
//       </div>
//       <AdmitCardDataGet folder_id={folderId} admin_id={admin.id} batch_id={batchId} refreshKey={refreshKey}/>
//       <Result folder_id={folderId} admin_id={admin.id} batch_id={batchId} refreshKey={refreshKey} onResultSubmitted={() => setRefreshKey(prev => prev + 1)} />
//       <ResultGetData folder_id={folderId} admin_id={admin.id} batch_id={batchId} refreshKey={refreshKey} />
...
function AdmitCard({ admit_card_folders_data, folderId, batchId }) {
  ...
  const handleBulkDateChange = (value) => {
    setBulkDate(value);
    setAdmitData((prev) => prev.map((item) => ({ ...item, exam_date: value })));
  };

  const handleBulkTimeChange = (value) => {
    setBulkTime(value);
    setAdmitData((prev) => prev.map((item) => ({ ...item, exam_time: value })));
  };
  ...
  const handleSubmitRow = async (row) => {
    try {
      ...
      console.log("Saved successfully:", response.data);
      alert("Row saved successfully!");
      setRefreshKey((prev) => prev + 1); // 🔥 refresh admit + result get data
    } catch (error) {
      console.error("Error saving row:", error);
      alert("Failed to save row!");
    }
  ...
  return (
    <div>
        ...
        <table className="min-w-full border border-gray-300">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="border px-4 py-2">Exam Venue</th>
      ...
      <Result
        ...
        batch_id={batchId}
        refreshKey={refreshKey}
        onResultSubmitted={() => setRefreshKey((prev) => prev + 1)}
      />
